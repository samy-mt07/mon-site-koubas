...
const ERROR_MESSAGES = {
  ...
  INVALID_QUANTITY: "Un ou plusieurs produits ne sont plus disponibles.",
  INSUFFICIENT_STOCK: "Un produit de votre panier n'a plus assez de stock disponible.",
  ...
  PREVIEW_FAILED: "Impossible de calculer le récapitulatif pour le moment.",
};
...
function CheckoutPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(ERROR_MESSAGES[data.error] || ERROR_MESSAGES.PREVIEW_FAILED);
      }

      setPreview(data);
      setStep("recap");
    } catch (err) {
      setSubmitError(err.message || ERROR_MESSAGES.PREVIEW_FAILED);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="checkoutPageContainer">
        ...
        {step === "form" ? (
          <div className="checkoutGrid">
            <form className="checkoutForm" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="checkoutSubmitButton" disabled={loading}>
                {loading ? "Calcul du récapitulatif…" : "CONTINUER"}
              </button>
            </form>
          ...
          <OrderRecap
            ...
            freeDelivery={preview.freeDelivery}
            total={preview.total}
            error={submitError}
            confirming={loading}
